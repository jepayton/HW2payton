...
>https://github.com/jepayton/HW2payton
...
```
```{bash}
head -10 data/county.csv
```

```{bash}
head -10 data/county_MI.csv
```
...
> ANSWER: Each row represents censuse data including population and additional variables for counties in the U.S and State of MI.
...
> ANSWER: Yes, one NA value only in county_df. No NA values in county_MI.

* What differences do you notice between the structure of the two data frames?
> ANSWER: county_MI has an additional variable (peninsula) for a total of 41 variables; county_df has only 40 variables.
...
>Distribution is positively skewed with a value of 14, a mean value of $46K exceeding the median value of $45K.


```{r}
ggplot(data=county_df) +
  geom_histogram(aes(x=Income), 
                bins=100, fill="steelblue", color="black") +
                ggtitle("Distribution of Median Income For U.S. Households") +
                xlab("Median Income")
```

```{r}
ggplot(county_df, aes(y=Income, x=1)) +
  geom_boxplot() +
  ggtitle("Distribution of Median Income For U.S. Households")
```
...
```{r}
ggplot(county_df, aes(y=Income, x=Region)) +
  geom_boxplot() + 
  ggtitle("Distribution of Median Income For U.S. Households By Region")
```

```{r}
ggplot(county_df, aes(y=Income, x=1)) +
  geom_violin() + 
  ggtitle("Median Income For U.S Households") +
  ylab("Median Income")
```
...
```{r}
ggplot(county_df) +
  geom_density(aes(x=Income), fill="steelblue") +
  ggtitle("Distribution of Median Income For U.S Households") + 
  xlab("Median Income")
```
...
```{r}
skewness(county_df$TotalPop)
var(x = county_df$Income, y = NULL, na.rm = TRUE)
sd(county_df$Income,na.rm = TRUE)
...
```{r histo_totpop}
ggplot(data=county_df) +
  geom_histogram(aes(x = TotalPop), bins=100, fill="steelblue", color="black") +
...
>Yes, there is much more variation in the income distribution for LP counties.

```{r vp_inc_by_peninsula}
ggplot(county_MI, aes(y = Income, x = peninsula)) +
  geom_violin() +
  ggtitle("Distribution of Median Income For Michigan Households By Peninsula") +
  xlab("Lower Peninsula/Upper Peninsula") +
  ylab("Median Income")
```
...
>ANSWER: Lower Peninsula has higher median unemployment but less variation. Upper Peninsula has lower median unemployment but more variation as compared to the Lower Peninsula.

```{r boxplots}
ggplot(county_MI, aes(y = Unemployment, x = peninsula)) +
  geom_boxplot() + 
  ggtitle("Distribution of Unemployment MI Households By Peninsula")
```


```{r}
ggplot(county_MI, aes(y = WorkAtHome, x = peninsula)) +
  geom_boxplot() + 
  ggtitle("Distribution of WorkAtHome MI Households By Peninsula")
```



```{r}
ggplot(county_MI, aes(y = SelfEmployed, x = peninsula)) +
  geom_boxplot() + 
  ggtitle("Distribution of Self Employed MI Households By Peninsula")
```
...
```{r faceted_histo}
ggplot(data=county_df) +
  geom_histogram(aes(x=Unemployment), bins = 100, fill="steelblue", color = "black") +
  facet_wrap(~Region) +
  ggtitle("Distribution of Unemployment For U.S. Households By Region")
```

Now do a group of boxplots of mean commute time by region.

```{r bp_commute_region}
ggplot(county_df, aes(y = MeanCommute, x = Region)) +
  geom_boxplot() + 
  facet_wrap(~Region) +
  ggtitle("Distribution of Mean Commute U.S. Households By Region") 
...
ggplot(data = county_MI, aes(x = Income, y = MeanCommute)) +
  geom_point() +
  ggtitle("Mean Commute for Michigan Households by Median Income") +
  xlab("Median Income - Michigan Households")
```
...
>ANSWER: There is no relationship indicated between median income and mean commute for Lower Peninsula counties. There appears to be a potential negative relationship between median income and mean commute for Upper Peninsula counties.
...
> ANSWER: Both the Upper Peninsula and Lower Peninsula display a negative relationship between median income and child poverty. The relationship of the Lower Peninsula appears to have a slightly greater slope.

```{r xy_faceted_peninsula}
ggplot(county_MI, aes(Income, ChildPoverty)) +
  geom_point(aes(color = peninsula)) +
  facet_wrap(~peninsula) +
  ggtitle("Distribution of Child Poverty By Income In MI") +
  xlab("Median Income - Michigan Households")
```

>ANSWER: Data indicate no relationship between Self Employed Households and Child Poverty rate for MI households.

```{r}
ggplot(county_MI, aes(SelfEmployed, ChildPoverty)) +
  geom_point(aes(color = peninsula)) +
  facet_wrap(~peninsula) +
  ggtitle("Distribution of Child Poverty By SelfEmployed In MI") +
  xlab("Self Employed - Michigan Households")
```
...
```{r xy_pop_inc}
ggplot(county_df, aes(TotalPop, Income)) +
  geom_point() +
  ggtitle("Distribution of Median Income For U.S. Households") +
  xlab("Total Population - U.S. Households") +
  ylab("Median Income")
```
...
>ANSWER: Log transforming provides a better view of the data.  Variables are spread out to allow detection of relational patterns.
...
ggplot(county_df, aes(TotalPop, Income)) +
  ...
  scale_y_continuous(breaks = seq(1000, 125000, by = 10000))              
```
